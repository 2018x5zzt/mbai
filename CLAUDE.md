...
mbai（赛博嘴脸图鉴）是一个纯前端轻娱乐人格测试站点。用户通过 20 道单选题，在浏览器本地计算出自己的"AI 使用人格"，生成可截图传播的结果卡。
...
```
src/
├── main.ts            # 状态管理 + UI渲染 + 事件绑定（766行，待拆分）
├── style.css          # 全局样式
├── types.ts           # 类型定义（67行）
└── lib/
    └── evaluate.ts    # 计分引擎（224行）

data/                  # 静态数据（JSON）
├── master-personalities.json   # 12 主人格
├── questions.json              # 20 题 × 4 选项
├── sub-tags.json               # 8 副标签
...
| 副标签 (SubTag) | 8 | 恶习叠加：SPAM/FAKE/TROLL/CHECK/SHOW/RAGE/CITE/LAZY |
...
每题每选项对各维度贡献 -2 ~ +2 分。最终 control 和 emotion 归一化到 [0,1] 形成二维坐标。

### 计分流程

1. 累加 20 题选项的维度分和 masterWeights
2. 归一化 control/emotion → userPoint
3. 对每个主人格计算 `masterWeights[code] - distance(userPoint, dimension) * 5`
4. 取最高分为结果人格
...
1. `main.ts` 766 行，混合了状态管理、3 个页面渲染、事件绑定、工具函数，应按职责拆分
